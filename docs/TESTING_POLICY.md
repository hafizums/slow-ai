...
fields, expose client-side timeline filter controls over safe timeline state,
avoid reconstructing timeline rows from `get_history`, and confirm the guest
shared page does not reference the internal timeline API or timeline filter UI.
The browser test opens real Canvas and Public Tool run details, waits for the
